...
const SquareCard = ({ icon, title, desc, color = 'primary' }) => {
  ...
  return (
    <div className={`aspect-square bg-white rounded-xl p-3 shadow-sm border border-neutral-100 flex flex-col items-center justify-center text-center ${cardHover}`}>
      <div className={`w-10 h-10 ${c.bg} rounded-xl flex items-center justify-center mb-2.5`}>
        {icon}
      </div>
      <h3 className="font-semibold text-neutral-800 text-sm mb-1 line-clamp-1">{title}</h3>
      <p className="text-xs text-neutral-500 line-clamp-2 leading-tight">{desc}</p>
    </div>
  );
};
          ...
          return (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
                <p className="text-white/60 text-xs mb-0.5">{track.desc}</p>
    ...
    <div className="min-h-screen bg-white">
      <section className="relative h-[85vh] max-h-[600px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-5 py-8 text-center">
          <div className="hero-badge mb-4 inline-flex items-center rounded-full px-3 py-1 text-xs text-white/90 font-medium">
          ...
          <p className="text-base text-white/80 mb-5 line-clamp-2">
            探索AI无限可能，开启创新之旅
          </p>
          <div className="flex flex-col gap-2.5 max-w-[220px] mx-auto">
            <Link 
              to="/register-competition" 
              className={`w-full bg-primary text-white py-3 rounded-lg font-medium text-base text-center ${btnHover}`}
            >
              立即报名
            </Link>
            <Link 
              to="/competition-center"
              className="w-full border border-white/20 bg-white/10 text-white/80 py-3 rounded-lg font-medium text-base text-center backdrop-blur-sm hover:bg-white/18 hover:text-white hover:border-white/30 transition-all duration-300"
            >
              查看赛事
            </Link>
          </div>
      ...
      <section className="py-5 px-4">
        <ScrollReveal threshold={0.1} rootMargin="-20px 0px">
          <h2 className="text-base font-bold text-neutral-800 mb-3">大赛亮点</h2>
        </ScrollReveal>
        <ScrollRevealStagger staggerDelay={0.08} threshold={0.1} rootMargin="-20px 0px">
      ...
      <section className="py-5 px-4">
        <ScrollReveal threshold={0.1} rootMargin="-20px 0px">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/eco-products" className={`text-sm text-primary font-medium ${linkHover}`}>更多→</Link>
          </div>
        </ScrollReveal>
        <ScrollRevealStagger staggerDelay={0.08} threshold={0.1} rootMargin="-20px 0px">
          <div className="grid grid-cols-2 gap-3">
      ...
      <section className="py-5">
        <ScrollReveal threshold={0.1} rootMargin="-20px 0px">
          <h2 className="text-base font-bold text-neutral-800 mb-3 px-4">专项赛道</h2>
        </ScrollReveal>
        <MobileTrackScroll />
      </section>
      ...
      <section className="py-5 px-4">
        <ScrollReveal threshold={0.1} rootMargin="-20px 0px">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/competition-center" className={`text-sm text-primary font-medium ${linkHover}`}>更多→</Link>
          </div>
        </ScrollReveal>
        <ScrollRevealStagger staggerDelay={0.1} threshold={0.1} rootMargin="-20px 0px">
          <div className="space-y-2.5">
            <Link to="/competition/1" className={`block bg-white rounded-lg p-3 shadow-sm border border-neutral-100 ${cardHover}`}>
              <div className="flex justify-between items-center gap-2">
                <h3 className="font-medium text-neutral-800 text-sm flex-1 line-clamp-1">2025年梧桐·鸿鹄人工智能应用创新大赛</h3>
                <span className="px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full flex-shrink-0">进行中</span>
              </div>
            </Link>
            <Link to="/competition/2" className={`block bg-white rounded-lg p-3 shadow-sm border border-neutral-100 ${cardHover}`}>
              <div className="flex justify-between items-center gap-2">
                <h3 className="font-medium text-neutral-800 text-sm flex-1 line-clamp-1">2025年梧桐·鸿鹄AI算法挑战赛</h3>
                <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full flex-shrink-0">即将开始</span>
              </div>
            </Link>
          </div>
        </ScrollRevealStagger>
      ...
      <section className="py-5 px-4">
        <ScrollReveal threshold={0.1} rootMargin="-20px 0px">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/news" className={`text-sm text-primary font-medium ${linkHover}`}>更多→</Link>
          </div>
        </ScrollReveal>
        <ScrollRevealStagger staggerDelay={0.1} threshold={0.1} rootMargin="-20px 0px">
          <div className="space-y-2">
            {[
              ...
              <Link key={index} to="/news/1" className={`flex items-center justify-between py-2.5 border-b border-neutral-100 last:border-0 ${linkHover}`}>
                <span className="text-sm text-neutral-700 line-clamp-1">{item.title}</span>
                <span className="text-xs text-neutral-400 flex-shrink-0 ml-2">{item.date}</span>
              </Link>
            ))}
          </div>
        </ScrollRevealStagger>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className={`aspect-square bg-neutral-50 rounded-lg flex items-center justify-center ${cardHover}`}>
                <span className="text-xs text-neutral-400">合作{i}</span>
              </div>
            ))}
          </div>
        </ScrollRevealStagger>
      ...
      <section className="py-6 px-4 pb-8">
        <ScrollReveal threshold={0.1} rootMargin="-20px 0px">
          <div className="bg-gradient-to-br from-primary/10 to-purple-50 rounded-xl p-5 text-center">
            ...
            <p className="text-sm text-neutral-600 mb-4">展示才华，开启AI精彩旅程</p>
            <Link 
              to="/register-competition" 
              className={`inline-block w-full bg-primary text-white py-3 rounded-lg font-medium text-base ${btnHover}`}
            >
              立即报名
            </Link>
          </div>
